...
- marts.mart_game_features has expected date range and row counts
...
# Same as ML assets: SQLMesh "local" uses marts__local.mart_game_features. Override for prod.
MART_FEATURES_TABLE = os.getenv("MART_FEATURES_TABLE", "marts__local.mart_game_features")
...
def main():
    ...
    print(f"(Training uses game_date >= {TRAINING_START} from {MART_FEATURES_TABLE})")
    print("=" * 70)

    errors = []
    warnings = []

    # 1. Mart: date range and row counts
    print(f"\n1. {MART_FEATURES_TABLE} — date range and trainable rows")
    try:
        rows = _run("""
            SELECT
        ...
        if "does not exist" in str(e).lower() or "relation" in str(e).lower():
            errors.append(f"{MART_FEATURES_TABLE} does not exist (table missing).")
        else:
            errors.append(f"{MART_FEATURES_TABLE} query failed: {e}")
        rows = []
    if rows:
        r = rows[0]
        ...
        if total == 0:
            errors.append(f"{MART_FEATURES_TABLE} is empty.")
        else:
    ...
    # 2. Rows per season
    print(f"\n2. {MART_FEATURES_TABLE} — rows per season (trainable only)")
    try:
        rows2 = _run("""
            SELECT
    ...
    # 3. Momentum JOIN completeness (mart already includes momentum; this checks intermediate if needed)
    mom_table = os.getenv("MOMENTUM_TABLE", "intermediate__local.int_game_momentum_features")
    print(f"\n3. Momentum — games in training range missing from {mom_table}")
    try:
        rows3 = _run("""
            SELECT COUNT(*) AS missing_momentum
            FROM """ + MART_FEATURES_TABLE + """ gf
            LEFT JOIN """ + mom_table + """ mf ON mf.game_id = gf.game_id
        ...
        if "does not exist" in str(e).lower():
            errors.append(f"{mom_table} or {MART_FEATURES_TABLE} missing.")
        else:
    ...
    # 4. Column count (schema and table from MART_FEATURES_TABLE, e.g. marts__local.mart_game_features)
    _schema, _table = MART_FEATURES_TABLE.split(".", 1) if "." in MART_FEATURES_TABLE else ("marts", "mart_game_features")
    print(f"\n4. {MART_FEATURES_TABLE} — column count")
    try:
        rows4 = _run("""
    ...
    # 5. Training query dry run (same filters as training asset: mart only, no JOIN)
